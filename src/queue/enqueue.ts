...
export async function enqueueReviewJob(data: any) {
  await reviewQueue.add("review", data, { removeOnComplete: true, removeOnFail: false });
}

export async function enqueueCommentReplyJob(data: any) {
  await reviewQueue.add("comment-reply", data, { removeOnComplete: true, removeOnFail: false });
}
